...
public class HashAlgorithmTest
{
    ...
    private static void ComputeHashStringTest(IHashAlgorithm hashAlgorithm, string str, string result)
    {
        var bytes = str.GetUtf8Bytes();
        var ms = new MemoryStream(bytes);
...
#if !NET48
    private static async Task ComputeHashAsyncTest(IHashAlgorithm hashAlgorithm, string str, string result)
    {
        var bytes = str.GetUtf8Bytes();
        var ms = new MemoryStream(bytes);
